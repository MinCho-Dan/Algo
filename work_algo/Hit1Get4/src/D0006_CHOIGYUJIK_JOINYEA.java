...
/*
 * 목적공을 일직선이 아닌 1도정도 틀어지게 만들어서 맞추는 코드입니다.
 * 
 * 
 */
...
public class D0006_CHOIGYUJIK_JOINYEA {

	// 닉네임을 사용자에 맞게 변경해 주세요.
	static final String NICKNAME = "D0006_CHOIGYUJIK_JOINYEA";
...
//				
//				for (int i = 0; i < 6; i++) {
//					float x = (int) Math.abs(targetBall_x - HOLES[i][0]);
//					float y = (int) Math.abs(targetBall_y - HOLES[i][1]);
//					float hole = x+y;
//					if (temp > hole) holeNum = i;
//				}
//				
//				int holeX = HOLES[holeNum][0];
//				int holeY = HOLES[holeNum][1];
//				
//				float w = Math.abs(targetBall_x - whiteBall_x);
//				float h = Math.abs(targetBall_y - whiteBall_y);
//				double radianHole = h > 0 ? Math.atan(w / h): 0;
//				float angleHole = (float) ((180.0 / Math.PI) * radianHole);
				...
				if (whiteBall_x < targetBall_x && whiteBall_y < targetBall_y)
				{
					...
					angle = (float) (((180.0 / Math.PI) * radian) + 1);
					System.out.println("1사분면에 있음");
				}
				...
				else if (whiteBall_x > targetBall_x && whiteBall_y < targetBall_y)
				{
					...
					angle = (float) (((180.0 / Math.PI) * radian + 271));
					System.out.println("2사분면에 있음");
				}
				...
				if (whiteBall_x == targetBall_x)
				{
					...
					{
						angle = 1;
						System.out.println("위에 있음");
					}
					else
					{
						angle = 179;
						System.out.println("아래에 있음");
					}
				}
